  ...
  module Exchange
    ...
    class APIClient
      ...
      def payment_methods(params = {})
        out = nil
        get("/payment-methods", params) do |resp|
          out = response_collection(resp)
          yield(out, resp) if block_given?
        end
        out
      ...
      def create_deposit(account_id, amt, params = {}, endpoint = 'coinbase-account', coinbase_account_type = :coinbase_account_id)
        params[:type] = "deposit"
        params[coinbase_account_type] = account_id
        params[:amount] = amt
        
        out = nil
        post("/deposits/#{endpoint}", params) do |resp|
          out = response_object(resp)
          yield(out, resp) if block_given?
        end
        out
      ...
      def create_withdraw(account_id, amt, params = {}, endpoint = 'coinbase-account', coinbase_account_type = :coinbase_account_id)
        params[:type] = "withdraw"
        params[coinbase_account_type] = account_id
        params[:amount] = amt

        out = nil
        post("/withdrawals/#{endpoint}", params) do |resp|
          out = response_object(resp)
          yield(out, resp) if block_given?
        end
        out
